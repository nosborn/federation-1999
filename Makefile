...
.PHONY: fedtpd httpd login perivale-go workbench
fedtpd httpd login perivale-go workbench: bin/$(shell go env GOOS)-$(shell go env GOARCH)
	go build -o bin/$(shell go env GOOS)-$(shell go env GOARCH)/$@ -ldflags=-w ./cmd/$@

.PHONY: modemd perivale workbench-c
modemd perivale workbench-c: bin/$(shell go env GOOS)-$(shell go env GOARCH)
	make -C cmd/$@
...
.PHONY: clean
clean: clean-coverage clean-generated clean-ibgames clean-modemd clean-perivale clean-website
	$(RM) -r bin/
