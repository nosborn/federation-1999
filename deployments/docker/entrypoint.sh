...
if [ ! -e data/billing.sqlite ] && [ -n "${BUCKET_NAME:-}" ]; then
  : # TODO
fi
if [ -n "${FLY_REGION:-}" ]; then
  chown fed:fed data/billing.sqlite
fi
if [ -e data/billing.sqlite ]; then
  _backup="data/billing.sqlite-${timestamp}"
  sqlite3 -cmd '.bail on' -readonly data/billing.sqlite ".backup '${_backup}'"
  bzip2 -9 "${_backup}"
  if [ -n "${BUCKET_NAME:-}" ]; then
    aws s3 cp "${_backup}.bz2" "s3://${BUCKET_NAME}/${timestamp}/billing.sqlite.bz2" --no-progress
  fi
  unset _backup
fi
find data -maxdepth 1 -mtime +7 -name 'billing.sqlite-*.bz2' -type f -delete
