...
func TestSnarkSystemDataIntegrity(t *testing.T) {
	testInitializeGameWorld()

	t.Run("Name", func(t *testing.T) {
		if snarkSystem.Name() != "Snark" {
			t.Errorf("Expected name 'Snark', got '%s'", snarkSystem.Name())
		}
	})

	t.Run("Duchy", func(t *testing.T) {
		if snarkSystem.Duchy() != SolDuchy {
			t.Error("snarkSystem should belong to SolDuchy")
		}
	})

	t.Run("EventCount", func(t *testing.T) {
		expectedEvents := 8
		if n := len(snarkSystem.events); n != expectedEvents {
			t.Errorf("Expected %d events, got %d", expectedEvents, n)
		}
	})

	t.Run("LocationCount", func(t *testing.T) {
		expectedLocations := 94 - SPACESHIP_SIZE
		if n := len(snarkSystem.locations); n != expectedLocations {
	...
	t.Run("ObjectCount", func(t *testing.T) {
		expectedObjects := 10
		if n := len(snarkSystem.objects); n != expectedObjects {
	...
	maxLocation := uint32(len(snarkSystem.locations) + SPACESHIP_SIZE)
	...
	t.Run("ObjectLocationBounds", func(t *testing.T) {
		minLoc := uint32(SPACESHIP_SIZE + 1)
		for i, obj := range snarkSystem.objects {
