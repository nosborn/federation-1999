...
main(int argc, char *argv[]) {
  ...
  const struct hostent *remote_host = gethostbyaddr(reinterpret_cast<char *>(&remote_addr),
                                                    sizeof(remote_addr),
                                                    AF_INET);
  ...
  char login[1024];
  const int len = snprintf(login, sizeof(login), LINK_FORMAT,
                           uid,
                           argv[2],
                           remote_hostname,
                           atoi(argv[3]));
  assert(len <= static_cast<int>(sizeof(login)));
  fprintf(stderr, "perivale[%d]: %s\n", getpid(), login);
  ...
  snprintf(addr.sun_path, sizeof(addr.sun_path), LINK_SOCKET, IB_homeDir());
...
output(const unsigned char *buf, size_t len) {
  ...
  if (trace) {
    snprintf(tbuf, sizeof(tbuf), "[LEN=%zu]", len);
    output += tbuf;
  }
  ...
  while (i < len) {
    ...
    switch (state) {
      ...
      case tsSpyDepth:
        if (trace) {
          snprintf(tbuf, sizeof(tbuf), "[S=%u]", ch);
          output += tbuf;
        }
      ...
      case tsTrace:
        if (spyDepth == 0) {
          ...
          if (trace || wasTrace) {
            snprintf(tbuf, sizeof(tbuf), "[T=%d]", trace);
            output += tbuf;
          }
        }
        state = tsData;
