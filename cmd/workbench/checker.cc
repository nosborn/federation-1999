...
loadData() {
  ...
  if (fileSize > 0) {
    numLocations = std::min((long) (fileSize / sizeof(dblocation_t)),
                            (long) (8L + LOCATION_LIMIT));
    locationData = new dblocation_t[numLocations];
  ...
  if (fileSize > 0) {
    numEvents = std::min((long) (fileSize / sizeof(dbevent_t)),
                         (long) (EVENT_LIMIT));
    eventData = new dbevent_t[numEvents];
  ...
  if (fileSize > 0) {
    numObjects = std::min((long) (fileSize / sizeof(dbobject_t)),
                          (long) (OBJECT_LIMIT));
    objectData = new dbobject_t[numObjects];
...
_LocInfo() {
  ...
  for (size_t count = 1; count <= numLocations; count++) {
    if (locationData[count - 1].events[0]) {
      printf("Event %d called on entry to loc %2d - ",
             locationData[count - 1].events[0], count);
      _PrintBriefDesc(count);
    }

    if (locationData[count - 1].events[1]) {
      printf("Event %d called on wrong move in loc %2d - ",
