...
loadData() {
  ...
  if (fileSize > 0) {
    numLocations = std::min(fileSize / sizeof(dblocation_t), 8 + LOCATION_LIMIT);
  ...
  if (fileSize > 0) {
    numEvents = std::min(fileSize / sizeof(dbevent_t), EVENT_LIMIT);
  ...
  if (fileSize > 0) {
    numObjects = std::min(fileSize / sizeof(dbobject_t), OBJECT_LIMIT);
