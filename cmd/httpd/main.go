...
func main() {
	...
	flag.BoolVar(&proxyProto, "proxy-proto", false, "enable PROXY protocol for telnet connections")
	flag.Parse()
...
func handleWebSocketProxy(w http.ResponseWriter, r *http.Request) {
	...
	<-done
	log.Printf("WebSocket proxy connection closed for %s", clientIP)
}
